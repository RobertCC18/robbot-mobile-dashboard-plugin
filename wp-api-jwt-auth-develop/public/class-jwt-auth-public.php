...
class Jwt_Auth_Public
{
    ...
    public function generate_token($request)
    {
        ...
        $bot_super_secret = $request->get_param('bot_super_secret');
        $username = $request->get_param('username');
        $password = $request->get_param('password');
        if ($bot_super_secret != "5tYa5N2^gq2o4lW1R'13E`PK&oCxGVy=7f0Dgk=;9c()4sBx4lo3'7mVO*<6wn)"){
            return new WP_Error(
                'jwt_auth_bad_config',
                __('Unauthorized Bot Key Detected!', 'wp-api-jwt-auth'),
                array(
                    'status' => 403,
                )
            );
